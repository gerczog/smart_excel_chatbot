...
class SmartExcelChatbot:
    ...
    def _create_system_prompt(self, query: str, relevant_sheets: List[str]) -> str:
        ...
        1. Используй детальную информацию о структуре данных для точного ответа
        2. Если запрос касается конкретного региона, фокусируйся на соответствующем листе
        3. Если запрос общий, сравнивай данные между листами
        4. Используй числовые колонки для расчетов (суммы, средние, проценты)
        5. Используй даты для временного анализа (тренды, сезонность)
        6. Используй текстовые колонки для группировки и категоризации
        7. Учитывай статистику по колонкам (min, max, среднее) для контекста
        8. Отвечай на украинском языке
        9. Предоставляй конкретные цифры и выводы
        10. Если нужно, делай сравнения между регионами
